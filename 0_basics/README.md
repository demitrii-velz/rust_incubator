...
- What memory model [Rust] has? Is it single-threaded or multiple-threaded? Is it synchronous or asynchronous? What is the memory layout of the box and vector? What are heap and stack? Where, but on heap and stack data could live in RAM?
	- Модель памяти в Rust использует следующие механизмы:
		...
		- Lifetime (Время жизни) - Каждый Владелец и каждое Заимствование имеет lifetime, который даёт возможность компилятору отлавливать нарушения заимствования - когда заимствование переживает владельца.
	...
	- Участки памяти:
		...
		- Stack - фиксированный участок памяти, предназначенный для хранения локальных переменных и аргументов вызова функции.
...
- What runtime [Rust] has? Does it use a GC (garbage collector)?
	- Rust использует libc как часть своего runtime, также в бинарном файле могут присутствовать функции сгенерированные LLVM.
...
- What statically typing means? What is a benefit of using it? Weak typing vs strong typing? Implicit / explicit?
	...
	- Позволяет отслеживать корректность взаимодействия данных на этапе компиляции и проводить оптимизации на основании типов.
	- Слабая типизации даёт больше гибкости во время исполнения программы, и меньше информации на этапе компиляции, также требует дополнительных механизмов во время исполнения.
	- Неявная типизация - механизм компилятора позволяющий выразить тип исходя из контекста использования данных.
- What are generics and parametric polymorphism? Which problems do they solve?
	- Механизмы позволяющие абстрагировать реализацию от конкретных типов данных.
...
- What are traits? How are they used? How do they compare to interfaces? What are an auto trait and a blanket impl? Uncovered type? What is a marker trait?
	- Трейт - механизм позволяющий изолировать нужное поведение от конкретного типа данных и при необходимости объявить для нужного типа это поведение.
	...
	- marker trait - трейт не имеющий методов, выступающий в роли метки, наличие которой можно отследить в compile time.
- What are static and dynamic dispatches? Which should I use, and when? What is monomorphisation?
	- Статическая диспетчеризация - компилятор подставляет вызов конкретной реализации параметрической функции для конкретного типа.
	- Динамическая диспетчеризация - компилятор подставляет обращение к виртуальной таблице, для взятия указателя на нужную реализацию функции.
	- Динамическая диспетчерезация используется когда нет точной информации на момент компиляции о том какой указатель нужно подставить, накладывая дополнительные расходы на вызов функции.
	- Мономорфизация - генерация компилятором варианта параметрической функции для каждого конкретного типа.
...
- What is immutability? What is the benefit of using it? What is the difference between immutability and const?
	...
	- const даёт гарантии что считываемое значение всегда актуально и неизменяемо со временем.
- What are move semantics? What are borrowing rules? What is the benefit of using them?
	- move semantics - передача владения в другой scope программы.
	- borrowing rules - правила по которым можно заимствовать объект.
...
- What are lifetimes? Which problems do they solve? Which benefits do they give?
	- lifetime - аналитическая отметка того как долго в программе живут данные.
...
- How code is tested in [Rust]? Where should you put tests and why?
	- Unit/Сomponent-тесты - реализуется в том же файле где и определен функционал.
...
- What is special about slice? What is layout of Rust standard data types? Difference between fat and thin pointers?
	...
	- Лейаут содержит информацию об количестве памяти занимаемых типом, и о том на какое число этот тип выравнен.
	- Тонкий указатель это непосредственно адрес ячейки в памяти, толстый указатель содержит указатель и дополнительную информацию о данных что за ним.
- Why [Rust] has `&str` and `String` types? How do they differ? When should you use them? Why str slice coexist with slice? What is differnece between `String` and `Vec`? 
	- &str - Толстый указатель на символы строки, String - аллоцированный буфер состоящий из символов.
	- &str может храниться на стеке. String всегда использует аллоцированную память.
	- Используются соответствующе со своим типом данных.
	- `&str` в отличии от слайса также как и `String` в отличии от `Vec`, не позволяет использовать прямую индексацию.
- Is [Rust] OOP language? Is it possible to use SOLID/GRASP? Does it have an inheritance? Is Rust functional language? What variance rules does Rust have?
	- Rust не является OOP language, так как нет возможности наследовать классы, но многие паттерны из OOP возможно имплементировать.
	- Да это возможно.
	- В Rust нет наследования структур, но есть возможность наследования в `trait`.
	- В Rust есть многие возможности из функционального программирования.
